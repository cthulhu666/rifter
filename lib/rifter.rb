...
module Rifter
  ...
  module Effects
    Dir[File.dirname(__FILE__) + '/rifter/models/effects/*.rb'].each do |file|
      base_name = File.basename(file).split('.').first
      # some classes violate the naming conventions (e.g. MissileDMGBonus) so we can't rely on base_name.camelcase
      klass_name = /class\s(.*?)\s/.match(File.read(file))[1]
      autoload klass_name, "rifter/models/effects/#{base_name}"
    end
  end

  module Traits
    Dir[File.dirname(__FILE__) + '/rifter/models/traits/*.rb'].each do |file|
      base_name = File.basename(file).split('.').first
      klass_name = base_name.camelcase
      autoload klass_name, "rifter/models/traits/#{base_name}"
    end
  end
